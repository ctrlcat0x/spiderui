...
export function UnderlineHover({
  ...
  const changePhase = useCallback((nextPhase: DrawPhase) => {
  ...
  const beginEnter = useCallback(() => {
    ...
    changePhase("enter");
  }, [availablePaths.length, changePhase]);
  ...
  const updateActiveState = useCallback(() => {
    ...
    if (nextActive) {
      if (phaseRef.current === "idle" || phaseRef.current === "leave") {
        beginEnter();
      } else {
        pendingLeaveRef.current = false;
      }
  ...
  }, [beginEnter, changePhase, reduceMotion]);
  ...
  const activePath =
    availablePaths[(activePathIndex ?? 0) % availablePaths.length] ?? "";
